...
export default async function CampaoAdvancedSimulationPage() {
  ...
  const registrations = await prisma.registration.findMany({ where: { championshipId: championship.id, category: { in: ["ADULTO", "MASTER"] } }, select: { id: true, fullName: true, birthDate: true, preferredPosition: true, level: true, category: true }, orderBy: { fullName: "asc" } });
  ...
  const age = (birthDate: Date) => Math.floor((Date.now() - birthDate.getTime()) / 31557600000);
  return <CampaoSimulationClient persistenceUnavailable={persistenceUnavailable} players={registrations.map((r) => ({ id: r.id, fullName: r.fullName, age: age(r.birthDate), position: r.preferredPosition, level: r.level, category: r.category! }))} relationships={relationships} simulations={simulations.map((s) => ({ ...s, updatedAt: s.updatedAt.toISOString() }))} />;
}
