...
export default async function CampaoAdvancedSimulationPage() {
  ...
  const registrations = await prisma.registration.findMany({ where: { championshipId: championship.id, category: { in: ["ADULTO", "MASTER"] } }, select: { id: true, fullName: true, birthDate: true, phone: true, preferredPosition: true, level: true, category: true }, orderBy: { fullName: "asc" } });
