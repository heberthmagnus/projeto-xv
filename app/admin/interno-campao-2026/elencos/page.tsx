...
import { addRosterPlayer, setRegistrationTeam, updateRosterTeamPresentation } from "./actions";
...
export default async function RostersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { filters, championship, registrations, sponsors } = data;
  ...
  const successMessage = filters.success === "jogador-adicionado" ? "Jogador incluído no elenco com um ID individual." : filters.success === "jogador-sem-time" ? "Jogador retirado do elenco e mantido no cadastro do campeonato." : filters.success === "time-atualizado" ? "Time do jogador atualizado com sucesso." : filters.success === "patrocinio-atualizado" ? "Patrocínio e camisa do time atualizados." : null;
  ...
  return <main className="xv-page-shell"><div className="xv-page-container space-y-5">
    ...
    <section className="space-y-4"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#8B6914]">Conferência</p><h2 className="mt-1 text-xl font-black">Elencos atuais</h2></div><div className="grid gap-4 lg:grid-cols-2">{teams.map((team) => <article key={team.id} className="xv-card"><h3 className="text-lg font-black">{team.icon} {team.shortName || team.name}</h3><p className="mt-1 text-sm text-[#6B7280]">{team.groupLabel || "Sem categoria"} · {team.players.length} jogador(es)</p><details className="mt-4 rounded-xl border border-[#E5E7EB] p-3"><summary className="cursor-pointer text-sm font-bold text-[#8B6914]">Patrocínio e camisa</summary><form action={updateRosterTeamPresentation} className="mt-3 grid gap-3"><input type="hidden" name="championshipTeamId" value={team.championshipTeamId}/><label className="grid gap-1 text-sm font-bold">Patrocinador da camisa<select name="sponsorId" defaultValue={team.sponsor?.id ?? ""} className="rounded-xl border border-[#D4D4D8] bg-white px-3 py-2.5 font-normal"><option value="">Sem patrocinador</option>{sponsors.map((sponsor) => <option key={sponsor.id} value={sponsor.id}>{sponsor.name}</option>)}</select></label><label className="grid gap-1 text-sm font-bold">Imagem da camisa<input name="shirtImageUrl" type="url" defaultValue={team.shirtImageUrl ?? ""} placeholder="https://..." className="rounded-xl border border-[#D4D4D8] px-3 py-2.5 font-normal"/></label><button className="rounded-xl bg-[#B89020] px-4 py-2.5 text-sm font-bold text-white">Salvar patrocínio</button></form></details><ul className="mt-4 divide-y divide-[#E5E7EB]">{team.players.map((player) => <li key={player.id} className="py-2 text-sm font-semibold text-[#374151]">{getPreferredPlayerName(player.registration.nickname, player.registration.fullName)}</li>)}</ul></article>)}</div></section>
  </div></main>;
}

async function loadRosterData(searchParams: Promise<SearchParams>) {
  const [filters, championship, registrations, sponsors] = await Promise.all([
    searchParams,
    prisma.championship.findUniqueOrThrow({
      ...
      select: {
        teams: {
          ...
          select: {
            id: true, groupLabel: true, shirtImageUrl: true,
            sponsors: { where: { isPrimary: true }, take: 1, select: { sponsor: { select: { id: true, name: true, logoUrl: true } } } },
            team: {
              select: {
  ...
  const decoratedChampionship = { ...championship, teams: championship.teams.map(({ id, groupLabel, shirtImageUrl, sponsors: teamSponsors, team }) => ({ groupLabel, team: { ...team, championshipTeamId: id, shirtImageUrl, sponsor: teamSponsors[0]?.sponsor ?? null } })) };
  return { filters, championship: decoratedChampionship, registrations, sponsors };
}
