...
export default async function InternoCampaoPublicPage() {
  ...
  return <main className="xv-page-shell-soft" style={{ padding: "12px 0 24px" }}><PageContainer><CampaoPublicDashboard teams={championship.teams.map((entry) => ({ category: entry.groupLabel === "MASTER" ? "MASTER" : "ADULTO", order: entry.displayOrder ?? 0, name: entry.team.shortName || entry.team.name, slug: entry.team.slug, icon: entry.team.icon, players: entry.team.players.map((player) => player.registration.nickname || player.registration.fullName) }))} matches={championship.matches.map((match) => ({ id: match.id, category: categoryByTeamId.get(match.homeTeam.id) ?? "ADULTO", round: match.round, order: match.roundNumber ?? 0, scheduledAt: match.scheduledAt?.toISOString() ?? null, status: match.status, homeScore: match.homeScore, awayScore: match.awayScore, home: match.homeTeam.shortName || match.homeTeam.name, homeSlug: match.homeTeam.slug, homeIcon: match.homeTeam.icon, away: match.awayTeam.shortName || match.awayTeam.name, awaySlug: match.awayTeam.slug, awayIcon: match.awayTeam.icon, scorers: match.participations.filter((item) => item.goals > 0).map((item) => ({ name: item.player.nickname || item.player.fullName, team: item.team.shortName || item.team.name, quantity: item.goals })), cards: [...match.participations.flatMap((item) => [{ name: item.player.nickname || item.player.fullName, team: item.team.shortName || item.team.name, quantity: item.yellowCards, type: "AMARELO" as const }, { name: item.player.nickname || item.player.fullName, team: item.team.shortName || item.team.name, quantity: item.redCards, type: "VERMELHO" as const }]).filter((item) => item.quantity > 0), ...match.events.map((item) => ({ name: item.player, team: item.team?.shortName || item.team?.name || "", quantity: item.quantity, type: "AZUL" as const }))] }))} /></PageContainer></main>;
}
